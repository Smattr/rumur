...
class ConstExprTraversal : public ConstBaseTraversal {
 ...
 public:
  void visit_add(const Add &n) override = 0;
  ...
  void visit_and(const And &n) override = 0;
  ...
  void visit_div(const Div &n) override = 0;
  void visit_element(const Element &n) override = 0;
  void visit_enum(const Enum &n) final;
  void visit_eq(const Eq &n) override = 0;
  void visit_errorstmt(const ErrorStmt &n) final;
  void visit_exists(const Exists &n) override = 0;
  void visit_exprid(const ExprID &n) override = 0;
  void visit_field(const Field &n) override = 0;
  void visit_for(const For &n) final;
  void visit_forall(const Forall &n) override = 0;
  void visit_function(const Function &n) final;
  void visit_functioncall(const FunctionCall &n) override = 0;
  void visit_geq(const Geq &n) override = 0;
  void visit_gt(const Gt &n) override = 0;
  void visit_if(const If &n) final;
  void visit_ifclause(const IfClause &n) final;
  void visit_implication(const Implication &n) override = 0;
  void visit_isundefined(const IsUndefined &n) override = 0;
  void visit_leq(const Leq &n) override = 0;
  void visit_lt(const Lt &n) override = 0;
  void visit_model(const Model &n) final;
  void visit_mod(const Mod &n) override = 0;
  void visit_mul(const Mul &n) override = 0;
  void visit_negative(const Negative &n) override = 0;
  void visit_neq(const Neq &n) override = 0;
  void visit_not(const Not &n) override = 0;
  void visit_number(const Number &n) override = 0;
  void visit_or(const Or &n) override = 0;
...
class ConstStmtTraversal : public ConstBaseTraversal {
 ...
 public:
  ...
  void visit_if(const If &n) override = 0;
  void visit_ifclause(const IfClause &n) final;
  ...
  void visit_procedurecall(const ProcedureCall &n) override = 0;
  void visit_property(const Property &n) final;
  void visit_propertyrule(const PropertyRule &n) final;
  void visit_propertystmt(const PropertyStmt &n) override = 0;
  void visit_put(const Put &n) override = 0;
  void visit_quantifier(const Quantifier &n) final;
  void visit_range(const Range &n) final;
  void visit_record(const Record &n) final;
  void visit_return(const Return &n) override = 0;
  void visit_ruleset(const Ruleset &n) final;
  void visit_scalarset(const Scalarset &n) final;
  void visit_simplerule(const SimpleRule &n) final;
  void visit_startstate(const StartState &n) final;
  void visit_sub(const Sub &n) final;
  void visit_switch(const Switch &n) override = 0;
  void visit_switchcase(const SwitchCase &n) final;
  void visit_ternary(const Ternary &n) final;
  void visit_typedecl(const TypeDecl &n) final;
  void visit_typeexprid(const TypeExprID &n) final;
  void visit_undefine(const Undefine &n) override = 0;
  void visit_vardecl(const VarDecl &n) final;
  void visit_while(const While &n) override = 0;
...
class ConstTypeTraversal : public ConstBaseTraversal {
 ...
 public:
  ...
  void visit_aliasstmt(const AliasStmt &n) final;
  void visit_and(const And &n) final;
  void visit_array(const Array &n) override = 0;
  void visit_assignment(const Assignment &n) final;
  void visit_clear(const Clear &n) final;
  void visit_constdecl(const ConstDecl &n) final;
  void visit_div(const Div &n) final;
  void visit_element(const Element &n) final;
  void visit_enum(const Enum &n) override = 0;
  void visit_eq(const Eq &n) final;
  void visit_errorstmt(const ErrorStmt &n) final;
  void visit_exists(const Exists &n) final;
  void visit_exprid(const ExprID &n) final;
  ...
  void visit_put(const Put &n) final;
  void visit_quantifier(const Quantifier &n) final;
  void visit_range(const Range &n) override = 0;
  void visit_record(const Record &n) override = 0;
  void visit_return(const Return &n) final;
  void visit_ruleset(const Ruleset &n) final;
  void visit_scalarset(const Scalarset &n) override = 0;
  void visit_simplerule(const SimpleRule &n) final;
  void visit_startstate(const StartState &n) final;
  void visit_sub(const Sub &n) final;
  void visit_switch(const Switch &n) final;
  ...
  void visit_ternary(const Ternary &n) final;
  void visit_typedecl(const TypeDecl &n) final;
  void visit_typeexprid(const TypeExprID &n) override = 0;
  void visit_undefine(const Undefine &n) final;
  void visit_vardecl(const VarDecl &n) final;
  void visit_while(const While &n) final;
