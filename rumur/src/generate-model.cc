    ...
    if (auto rule = dynamic_cast<const Rule*>(child.get())) {
      ...
      for (const Ptr<Rule> &r : rs) {
        ...
        if (auto p = dynamic_cast<const PropertyRule*>(r.get())) {
          ...
          out << "  static const char *rule_name __attribute__((unused)) = "
            "\"property " << rule_name_string(*p, property_index) << "\";\n";
        ...
        for (const Ptr<Rule> &r : rs) {
          if (isa<SimpleRule>(r)) {
            ...
            out << ")) {\n"
              ...
              << "            queue_id = thread_id;\n"
              << "\n"
              << "            /* If set expansion rebalanced our thread-load seen_count\n"
              << "             * by lowering it, it could now be below last_size_notified.\n"
              << "             * Avoid confusing progress output by accounting for this.\n"
              << "             */\n"
              << "            if (!PRECISE_SEEN_COUNT && size < last_size_notified) {\n"
              << "              last_size_notified = size;\n"
              << "            }\n"
              << "\n"
              << "            bool notification_due =\n"
              << "              (PRECISE_SEEN_COUNT && size % 10000 == 0) ||\n"
              << "              (!PRECISE_SEEN_COUNT && size - last_size_notified >= THREADS * 10000);\n"
              << "\n"
              << "            if (notification_due) {\n"
              ...
              << "              }\n"
              << "              /* update last_size_notified even if we failed to notify,\n"
              << "               * to avoid repeatedly contending on the printf lock\n"
              << "               */\n"
              << "              last_size_notified = size;\n"
              << "            }\n"
              << "\n"
